...
class Tag:
    ...
    def save(self):
        with dbapi2.connect(current_app.config['dsn']) as connection:
            ...
            statement = """INSERT INTO TAG (tag_name) VALUES (%s) RETURNING tag_id;"""
    ...
    def delete(self):
        with dbapi2.connect(current_app.config['dsn']) as connection:
            cursor = connection.cursor()
            statement = """DELETE FROM TAG WHERE (tag_id = %s);"""
    ...
    def update(self):
        with dbapi2.connect(current_app.config['dsn']) as connection:
            cursor = connection.cursor()
            statement = """UPDATE TAG SET tag_name = %s WHERE (tag_id = %s);"""
